(function (){
    ...
    function addFolder(){
        ...
        if(rname != null){
            rname = rname.trim(); // removes spaces from start and end
        }
        
        if(!rname){ // empty name valiation
    ...
    function deleteHelper(fidTBD){
        ...
        for(let i = 0; i < children.length; i++){
            deleteHelper(children[i].rid); // this is capable of delete children and their children respectively
        }
    ...
    function renameTextFile(){
        ...
        divName.innerHTML = nrname;
        // change ram
        let test = resources.find(r => r.rid == ridTBU);
        //resource.rname = nrname;
        test.rname = nrname;
        // change storage
        saveToStorage();
    ...
    function uploadNotepad() {
        let file = window.event.targe.files[0];
